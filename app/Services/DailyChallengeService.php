...
class DailyChallengeService
{
     ...
     */
    public function submit(User $user, DailyChallenge $challenge, array $input, array $timerState): array
    {
        ...
        return match ($challenge->game_type) {
            'letters' => $this->submitLetters($user, $challenge, $input, $timerState),
            'numbers' => $this->submitNumbers($user, $challenge, $input, $timerState),
     ...
     */
    private function submitLetters(User $user, DailyChallenge $challenge, array $input, array $timerState): array
    {
        ...
        return DB::transaction(function () use ($user, $challenge, $submittedWord, $score, $isPerfect, $timerState) {
            ...
            $gameSession = GameSession::query()->create([
                ...
                'score' => 0,
                'status' => 'started',
                'started_at' => $timerState['started_at'] ?? $completedAt,
                'expires_at' => $timerState['expires_at'] ?? $completedAt,
            ]);
     ...
     */
    private function submitNumbers(User $user, DailyChallenge $challenge, array $input, array $timerState): array
    {
        ...
        $evaluation = $this->numbersExpressionEvaluator->evaluateSubmittedSolution($submittedSolution, $numbers);
        ...
        return DB::transaction(function () use ($user, $challenge, $submittedSolution, $resultValue, $difference, $score, $isPerfect, $timerState) {
            $lockedChallenge = DailyChallenge::query()->lockForUpdate()->findOrFail($challenge->id);
            $existingAttempt = DailyChallengeAttempt::query()
                ->where('daily_challenge_id', $lockedChallenge->id)
                ->where('user_id', $user->id)
